...
export async function postBacktest(env: Env, req: Request, headers: HeadersInit) {
  ...
  const strategy = typeof payload.strategy === "string" ? payload.strategy : "sma";
  const id = crypto.randomUUID();
  await env.DB.prepare("INSERT INTO backtests (id, owner, strategy, params, r2_key, status) VALUES (?1,?2,?3,?4,?5,?6)")
    .bind(id, "unknown", strategy, JSON.stringify(payload), "", "queued")
    .run();
